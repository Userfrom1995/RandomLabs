...
// HTTPTunnelAddr returns host:port for HTTP CONNECT proxy clients.
func (in *Instance) HTTPTunnelAddr() string {
	if in == nil || in.HTTPTunnelPort <= 0 {
		return ""
	}
	return net.JoinHostPort("127.0.0.1", strconv.Itoa(in.HTTPTunnelPort))
}
...
type Options struct {
	...
	TempParent string
	// Detached indicates the instance will outlive the launching process.
	// In detached mode, Launch does not register the launching process as
	// the owning controller so that tor stays alive until the supervisor
	// terminates it.
	Detached bool
	// OnProgress is an optional callback that receives bootstrap state updates.
	OnProgress func(control.BootstrapState)
}
...
var managedTorrcKeys = map[string]bool{
	...
	"httptunnelport":       true, "httptunnelportwritetofile": true,
...
func GenerateTorrcTrans(dir string, extra []string, transPort, socksPort, dnsPort int) string {
	return GenerateTorrcFull(dir, extra, transPort, socksPort, dnsPort, 0)
}

// GenerateTorrcFull renders a private-instance torrc with optional fixed TransPort
// and optional fixed HTTPTunnelPort.
func GenerateTorrcFull(dir string, extra []string, transPort, socksPort, dnsPort, httpTunnelPort int) string {
	...
	if httpTunnelPort > 0 {
		fmt.Fprintf(&b, "HTTPTunnelPort 127.0.0.1:%d\n", httpTunnelPort)
	}
...
func Launch(o Options) (*Instance, error) {
	...
	// SOCKS/DNS/HTTPTunnel listeners use wrapper-picked fixed ports (real tor has no
	// SocksPortWriteToFile/DNSPortWriteToFile), chosen before the temp dir
	// so a pick failure needs no cleanup.
	socksPort, dnsPort, httpTunnelPort, err := pickListenerPorts3(o.TransPort)
	if err != nil {
		return nil, err
	}
	...
	if o.RunAs != nil {
		// Ensure parent directory is searchable by the unprivileged user.
		parent := filepath.Dir(abs)
		if fi, err := os.Stat(parent); err == nil {
			if fi.Mode().Perm()&0o011 != 0o011 {
				_ = os.Chmod(parent, fi.Mode().Perm()|0o011)
			}
		}
	}
	torrcPath := filepath.Join(abs, "torrc")
	if err := os.WriteFile(torrcPath, []byte(GenerateTorrcFull(abs, o.ExtraTorrc, o.TransPort, socksPort, dnsPort, httpTunnelPort)), 0o644); err != nil {
		os.RemoveAll(abs)
	...
	var cmd *exec.Cmd
	if o.Detached {
		cmd = exec.Command(o.TorBinary, "-f", torrcPath)
	} else {
		self := os.Getpid()
	...
	logF, err := os.OpenFile(filepath.Join(abs, "stdout.log"), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
	if err != nil {
		os.RemoveAll(abs)
	...
	cmd.Stderr = logF
	if o.RunAs != nil {
		// The child will run unprivileged, so hand it the directory and its contents first.
	...
	cleanup := func(err error) (*Instance, error) {
		...
		if logBytes, rerr := os.ReadFile(filepath.Join(abs, "stdout.log")); rerr == nil && len(logBytes) > 0 {
			tail := string(logBytes)
			if len(tail) > 1024 {
				tail = tail[len(tail)-1024:]
			}
			logSnippet = strings.TrimSpace(tail)
		}
		in.stopOwned()
		os.RemoveAll(abs)
		if logSnippet != "" {
			return nil, fmt.Errorf("%w (tor output: %s)", err, logSnippet)
		}
		return nil, err
	}
	...
	in.HTTPTunnelPort = httpTunnelPort
	...
	if err := WaitReadyWithProgress(ctl, deadline, o.PollInterval, o.OnProgress); err != nil {
		return cleanup(err)
	}
	saveCache(abs)
...
// pickListenerPorts3 returns distinct free loopback ports for the fixed
// SOCKS, DNS, and HTTPTunnel listeners, avoiding the fixed TransPort when enabled.
func pickListenerPorts3(transPort int) (socks, dns, httpTunnel int, err error) {
	for i := 0; i < 25; i++ {
		socks, err = pickFreePort()
		if err != nil {
			return 0, 0, 0, err
		...
		httpTunnel, err = pickFreePort()
		if err != nil {
			return 0, 0, 0, err
		}
		if socks != dns && socks != httpTunnel && dns != httpTunnel &&
			socks != transPort && dns != transPort && httpTunnel != transPort {
			return socks, dns, httpTunnel, nil
		}
	}
	return 0, 0, 0, fmt.Errorf("lifecycle: could not pick distinct free listener ports")
}
...
func WaitReady(ctl *control.Client, deadline time.Time, poll time.Duration) error {
	return WaitReadyWithProgress(ctl, deadline, poll, nil)
}

// WaitReadyWithProgress polls GETINFO until the readiness gate passes,
// optionally notifying onProgress on bootstrap progression or completion.
func WaitReadyWithProgress(ctl *control.Client, deadline time.Time, poll time.Duration, onProgress func(control.BootstrapState)) error {
	if poll <= 0 {
	...
	for {
		...
		if onProgress != nil && (last.Progress != lastReportedProgress || last.Ready()) {
			onProgress(last)
			lastReportedProgress = last.Progress
		}
		if last.Ready() {
			return nil
		}
