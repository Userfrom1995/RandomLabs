...
## Current step
Phase 3 complete: Workbook/Series/Format/Codecs plus property suites
(minimal-vs-full, Kahn residue vs DFS taint, insert-delete identity,
delete-undo, sticky REF, range clamp, paste commutation, CSV idempotence,
JSON round-trip, format purity) plus 10k-cell proxy full=121ms minimal=82ms
in debug (native proxy budget 250ms met). 61/61 green in 0.35s.
Next is Phase 4 (Bridge batch wire plus virtualized canvas grid UI).
...
- 2026-09-03 (Builder Phase 3): wrote `Workbook.swift` (sheets, entry coercion, names with cell-shape rejection, minimal/full recalc with ValueBox ordered pass, insert/delete rows/cols with base-only host shift plus target remap, single-taint/range-clamp rules, rename-follow plus sheet-delete taint, copy/paste translation, Series-backed fill, undo stack, styles), `Series.swift` (pure extend with numeric-step/trailing-number/copy laws), `Format.swift` (General/fixed/currency/percent/ISO-date display, recalc purity), `Codecs.swift` (canonical JSON with styles plus CSV/TSV with injection guard). Fixed three self-caught defects: mappingRefs overload self-recursion (stack overflow), range-endpoint base drift on host moves, and an O(V^2) `formulaSucc` key-intersection (10k proxy 6.7s to 27ms; precomputed sorted succ adjacency plus binary-heap Kahn). `Phase3Tests.swift` (24 suites incl. seeded minimal-vs-full, Kahn-vs-DFS, paste commutation, CSV idempotence, JSON round-trip, 10k proxy full=121ms minimal=82ms debug). 61/61 green in 0.35s. Decision action: continue (Phase 4 bridge plus grid UI).
