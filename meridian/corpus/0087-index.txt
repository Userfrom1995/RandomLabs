...
(typo-tolerant) retrieval, CJK ideographic segmentation, ranking signals, and
relevance snippets. The engine is mirrored one-to-one in dependency-free
JavaScript, so the static web UI searches a 110-document corpus of the
factory's own documentation entirely in the browser. Zero dependencies: the
crate is pure `std`, the mirror is pure ES5.
...
- **Corpus crawler.** Walks a directory deterministically (skipping hidden,
  ...
  repo: 110 documents, 122,799 tokens, 7,765 terms.
...
- **Concurrency & instrumentation.** `--threads N` parallelizes crawl/index
  with a deterministic merge, `--time` prints per-phase ms, and `meridian
  bench` runs a built-in query benchmark.
- **JS mirror.** `js/meridian.js` re-implements the engine in dependency-free
  JavaScript; a consistency harness runs 31 queries through both engines with
  both rankers and a stem/signals matrix and asserts identical doc sets,
  order, scores, matches, suggestions, and snippet highlights (9,296 checks).
...
node tests/consistency.test.mjs   # 9296 JS-vs-Rust checks
...
| `meridian search --corpus <dir> --query <q> [--scoring bm25\|tfidf] [--top N] [--format text\|json] [--stem on\|off] [--signals on\|off]` | Crawl (or reuse) the corpus and search it. |
