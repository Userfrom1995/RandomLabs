"""Tester regression suite for PR #353 soak 34-group sync (issue #302).

Guards the 34-group truth across every derived bundle rebuilt from
committed data: matrix.csv (34 rows: 27 measured + 7 timeout), raw/
(156 files over 34 tier-groups), report/supplement/sitemeta agreement,
manifest raw total 2395 = 150+319+1770+156, dossier source SHAs fresh,
and comparison.json showing exactly 2 "tier not in git" absents.

The absent set is exactly {(M10-S3, 3600, odyssey),
(M10-S3, 3600, pgcat)} against the 36-group geometry
(3 cells x 6 poolers x 2 tiers). Each test fails on stale 18-group
bundles and passes on the synced 34-group bundles; nothing here is
tautological. Authored by the Tester.
"""
...
TIERS = ["1800", "3600"]
ABSENT = {("M10-S3", "3600", "odyssey"), ("M10-S3", "3600", "pgcat")}
...
class TestSoak34GroupTruth(unittest.TestCase):
    """Matrix shape: 34 rows, 27 measured, 7 timeout, both tiers."""
...
class TestSoakRawCoverage(unittest.TestCase):
    """Raw corpus: 156 files (54 legacy untiered + 102 tiered).
    ...
    pool 3 legacy + 3 re-dispatch repeats (n=6); the 16 new groups
    have 3 repeats (n=3). 18*6 + 16*3 = 156.
    """
    ...
    def test_raw_file_count(self):
        ...
        self.assertEqual(len(tiered), 102)
...
class TestSoakBundleAgreement(unittest.TestCase):
    ...
    def test_comparison_absent_markers(self):
        with open(os.path.join(RESULTS, "charts",
                               "comparison.json")) as fh:
            comp = json.load(fh)
        self.assertEqual(json.dumps(comp).count("tier not in git"), 2)
