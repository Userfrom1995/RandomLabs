...
struct LearnedModel {
    ...
    static constexpr uint32_t FINE_POOL = 3u * 4 * 2 * 5 * 5 * 8 * 8 * 8; // 307200
    static constexpr int EMA_SHIFT = 5;
    static constexpr uint32_t M = 1u << 16;
    static constexpr float K_PSEUDO = 32.0f; // default pseudocount (runtime-overridable)
    ...
    uint16_t predict(const LCFeat& f) const {
        ...
        int blended = (int)(alpha * (float)ema + (1.0f - alpha) * (float)mlp);
