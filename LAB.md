...
- **Merge capability (workflows: write)**: Merging any PR that modifies `.github/workflows/*` requires the `workflows: write` scope on the merging token. The Maintainer merges from inside her session with the ambient `GITHUB_TOKEN`, so `maintainer.yml` (and every other workflow whose steps or agent may push, relink, or merge git state: `lab.yml`, `opencode.yml`, `opencode-recover.yml`) declares `workflows: write`. Read-only agents (Reviewer, Tester, Auditor, Ideator) deliberately do not carry the scope. Without it, infra PR merges die with "refusing to allow a GitHub App to create or update workflow ... without workflows permission" even when fully approved (live regression observed 2026-08-24 on PR #139; originally flagged in issue #120).
