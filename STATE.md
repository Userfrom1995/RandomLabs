...
- **Updated:** 2026-08-22 (maintainer run 32599170314, owner comment on PR #118). Re-survey confirms: PR #118 head `a6f361e480490739b5ae685767b58bbeb55f3c1f` (B5.42, 11.025 bpp, byte-exact), **genuine build in flight** (run `32599163762`, build job `in_progress` since 21:17:42Z) resuming B7 + further entropy from B5.42.
...
- **`main` = `91c87078919e17f7244a659b2cbf5552c4052502`** (owner commit "Remove halt/breaker on factory pipeline"). Obsidian lives in `obsidian/` on `main`. Prism branch `opencode/117-prism-m1-m4-optimization` = `a6f361e` shares M0 ancestry (NOT orphan).
...
## IN FLIGHT
- **Prism M1-M4 (issue #117, PR #118, branch `opencode/117-prism-m1-m4-optimization`):** head `a6f361e` (B5.42, **11.025 bpp**, byte-exact). **Genuine `continue` build in flight** (run `32599163762`, build job `in_progress` since 2026-08-22T21:17:42Z) resuming B7 + further entropy from B5.42.
  - **Trajectory:** The PREDICTOR/COLOR/BLOCK bank is saturated (16/16 nibble 0..15, per-plane top11, block top11, selective-16 thr45 top11). The RESIDUAL ENTROPY MODEL is still EXPANDABLE with real gains: B5.38 (704-context orientation split, -0.16%), B5.39 (2816-context flatness split, -0.11%), B5.41 (5632-context diagonal edge, -0.09%), B5.42 (11264-context sign coherence, -0.004%) - each a genuine but diminishing byte reduction on byte-exact Kodak. Total progress from 11.29 baseline is ~2.35%. The loop is productive, NOT converged.
...
## PENDING (in order)
    1. **Reach the JXL gate (M3 < 8.71).** Entropy context-splitting still yields ~0.1% gains (now at 11264 contexts via sign coherence); keep grinding it, but the ~21% gap needs the real B7 Squeeze+MA-tree greedy split (mandatory `llc_class`/`sibling_class`). The progress file lists B7 next, and B5.40 recovered the harness budget, so the in-flight `continue` should finally attempt B7. Since plain `continue` follows the progress file's next step, iteration can proceed.
    ...
    5. **Silent-stall mitigation:** now moot - owner removed the iteration cap and the breaker; loop resumes normally (this run found a genuine build in flight, no re-dispatch needed).
...
## NEXT STEPS
    1. **Build in flight (run `32599163762`)** resumes the loop from B5.42. The progress file lists B7 MA-tree greedy split as the next step, and B5.40 recovered the harness budget, so the in-flight build should finally attempt the real B7 (mandatory `llc_class`/`sibling_class`).
...
## OPEN QUESTIONS
- Will the in-flight `continue` (32599163762) finally attempt the real B7 MA-tree greedy split (mandatory `llc_class`/`sibling_class`), now that B5.42 expanded entropy to 11264 contexts and the progress file lists B7 next? Context-splitting is still productive but the ~21% JXL gap needs B7.
