...
## STANDING OWNER DIRECTIVES (active)
 ...
 - **LAB HEALTH NOTICE (2026-09-07T16:06Z via #70):** Documentation & Landing Page Sync - FIXED at 113966e1 + Curator syncs + Trigger-list fix at 0b16d0be + eval wiring at 1ff6eb09 + eval checkout mirror at 94991020 restores 16/16 + Deploy 35767191697 success; eval version-fetch hardening pending.
---

## CRITICAL INFRASTRUCTURE STATE
 - **Main 94991020 LIVE - version-fetch hardening needed:** `origin/main` = `949910204d6bacfb0723a9a96a06d2e590c555d6` verified (parent 1ff6eb09, rebase-merge of 8868a31), `gh pr list --state open` = [376] MERGEABLE (merge-base 1ff6eb09 present via 0b16d0be, linearizes), `gh issue list --state open` = [375 Umbra, 70 lab-health, 42 brainstorm], live workflows 19 vs allowlist 16/16 PASS (`maintainer.yml:38` includes opencode-eval, `maintainer.yml:198` includes test|eval, `maintainer.yml:548` elif eval -> /oc eval, `opencode-eval.yml:71` model muse-spark-1.3 free). Evaluator 35769769470 FAILED 18:49:45Z due to `curl -sf https://api.github.com/repos/anomalyco/opencode/releases/latest` exit 1 (unauthenticated 403/rate-limit, 183 ms, no decision) - Lab fix needed (auth GH_TOKEN/gh api + retry + fallback). Checkout mirror (Get PR info + Checkout PR head ref head_ref, pull-requests write, yaml.safe_load, bash -n) already PASS.
...
## IN FLIGHT
 - **Doom #362 CLOSED + PRs #363-#367 MERGED (M1-M5) + PR #368 MERGED (docs) + PR #370/#372 MERGED (Curator) + Evaluator bootstrap at 5f15cef6 + Lab PR #374 MERGED at 0b16d0be + Lab PR #377 MERGED at 1ff6eb09 + Lab PR #378 MERGED at 94991020:** Epic complete. Trigger-list 16/16 PASS + eval wiring + eval checkout mirror all live. Version-fetch auth hardening pending.
 - **Umbra #375 OPEN - Lab HARDENING DISPATCHED on PR #376:** Blueprint + `progress/375-umbra.md` on branch head (M1-M5, Status: in-progress, Active Milestone: M1, Refs intermediates). PR #376 `opencode/issue375-20260922160615` at c19a3fe8 OPEN, Refs #375. Fixer f55274c + Tester c19a3fe8 63/63, Reviewer APPROVED 16:24:55Z, Tester APPROVED 16:29:39Z, Evaluator FAILED 18:04:53Z (1ff6eb09, no decision) + 18:49:45Z (94991020, curl version fetch 403, no decision) -> Dispatch `{"action":"lab","pr":376}` to harden `opencode-eval.yml` Get opencode version (GH_TOKEN auth, gh api fallback, retry, graceful fallback to latest) -> rerun `{"action":"eval","pr":376}` >=9.8 before Refs #375 merge and M2 chain; keep #375 OPEN until final M5.
 - **Open PRs:** [376 Umbra M1 at c19a3fe8 OPEN awaiting lab hardening + eval rerun on 94991020]
 - **Open issues:** #375 Umbra (M1 awaiting Evaluator >=9.8 after version-fetch fix, 7 binding gates) + #70 lab-health + #42 brainstorm.
 - **Lab wiring partially live on main 94991020:** maintainer.yml eval wiring + opencode-eval.yml model unify at 1ff6eb09 + opencode-eval.yml checkout mirror at 94991020 (Get PR info via OPENCODE_PAT + Checkout PR head ref head_ref + pull-requests write, yaml.safe_load, bash -n) — PASS; version-fetch auth fix pending.
---

## PIPELINE POSITION
 Doom M1 REJECT 5.3 -> fix -> APPROVE 9.84 -> MERGE -> M2 8.0 -> fix -> 9.8 MERGE -> M3 fix NaN -> 9.86 MERGE -> M4 9.88 MERGE -> M5 MERGED at e33e11f1, Doom epic COMPLETE + Evaluator bootstrap at 5f15cef6 + Lab fix at 0b16d0be 16/16 PASS + Lab eval wiring at 1ff6eb09 16/16 + eval knob live BUT eval fail-closed twice 35764829287 on 1ff6eb09 + 35769769470 on 94991020 (both missing decision, last 183 ms curl 403) -> Lab checkout mirror at 94991020 SUCCESS (8868a31 rebase-merged, Deploy 35767191697 success) BUT version fetch still unauthenticated. **Current: main 94991020 LIVE, trigger-list 16/16 PASS + eval checkout mirror, two-knob free unified, PR #376 M1 at c19a3fe8 Reviewer APPROVED + Tester APPROVED 63/63 + Lab hardening DISPATCHED to fix version fetch before eval >=9.8 rerun.**
---

## NEXT-RUN PLAYBOOK
 1. Verify Lab PR on PR #376 hardening `opencode-eval.yml` Get opencode version (GH_TOKEN auth + gh api fallback + retry + graceful fallback, plus existing Get PR info + Checkout PR head, model muse-spark-1.3, yaml.safe_load, bash -n) landed and merged to new main successor, Deploy success.
...
## ISSUES
 - **#375** - OPEN Umbra (Lab hardening dispatched on PR #376 for eval version-fetch 403, awaiting >=9.8, 7 binding gates)
 - **#376** - OPEN Umbra M1 at c19a3fe8, Reviewer APPROVED 16:24:55Z f55274c, Tester APPROVED 16:29:39Z 63/63, Evaluator FAILED 18:04:53Z + 18:49:45Z (curl version fetch 403, no decision) -> lab hardening, Refs #375
 - **#378** - MERGED at 18:26:40Z to 94991020 (lab checkout mirror, 8868a31, 1 file opencode-eval.yml)
 - **#70** - OPEN lab-health (16/16 PASS + eval checkout mirror + Deploy 35767191697 success, version-fetch hardening pending)
