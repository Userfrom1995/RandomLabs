...
- **Updated:** 2026-08-24 (~19:45Z, maintainer run 32769657984 queued from 19:41:59 trigger on PR #142; survey at 19:45Z after pipeline auto-forwarded). PR #142 head 6d778a9 APPROVED routing to Tester; D4 live on #131; #139 blocked awaiting owner; #134 draft hold unchanged.
...
- **`main` = `630dce19bf86268650f73bad2083fdd0fc262bac`** (19:30Z era owner direct model-assignment series "fix: update model assignments for all agents"). Pages green (action_required pairs for 6d778a9 at 19:44:15Z pending sweep, normal). Do NOT dispatch lab to edit workflows while the owner works on them - collision risk. Re-survey main tip next run before any base-relative claim.
- **Model watch:** openrouter/stealth/ox-alpha active; transient rate-limit at 19:18-19:22Z self-healed; this window's review/lab/test chain completed clean end-to-end. No model switch needed; two-knob rule (opencode.json small_model) remains if ever retried.

## IN FLIGHT
- **PR #142** - OPEN MERGEABLE head `6d778a9b0fa078ddc6303d6a4f71328849b573d3` (branch `opencode/issue70-20260824184700`, base `630dce19bf`). Prior head `20fa0afec9bb9f3f9326405022e9e09050f6c466` dispatched to Reviewer at 19:41:45Z (this run's predecessor). Review round 1 (19:43:16Z) routed `lab` for Finding 1 (`Closes #141` links a closed PR not an issue). Lab Engineer landed fix at 19:44:09Z: `6d778a9` "lab: relink pr body off the dead predecessor" + `gh pr edit 142` body -> `Continues #141 / Fixes #120 / Refs #70` (verified via gh api: #141 is pull_request closed, #120 closed issue, #70 open board). Review round 2 at 6d778a9 APPROVED 19:45:00Z (14 checks pass, docs content verified: AGENTS.md:62 + LAB.md:73 PAT merge path, no `workflows: write` claim) routing to Tester via `{"action":"test"}`; owner posted `/oc test` 19:45:04Z. Tester run 32769959953 IN_PROGRESS at survey time. Next maintainer run merges on `approve-test` with fresh orphan check (expected PASS: merge-base 630dce19bf, 2 ahead). Held PR preview + pr-trigger pairs for 6d778a9 `action_required` await PAT sweep - normal bot-PR flow.
...
- **PR #131** - OPEN but now CONFLICTING (dirty) head `cb639e2bbedfad94da99714183e1f6c364f3a705` (`opencode/issue130-20260823163248`, researcher Prism gap analysis + D1 gate fix). D4 continuation dispatched earlier (run 32769431506) still referenced in prior STATE; opencode run 32769810484 shows in_progress at survey (title "researcher: Prism gap analysis..."), but branch state dirty suggests rebase needed after main's model-pin advances. Parked at OWNER DECISION POINT regardless of D4 outcome; freeze blocks any merge; next review automatic at phase boundary.
...
## NEXT-RUN PLAYBOOK
1. FIRST ACTION: read Tester verdict on #142 (run 32769959953). `approve-test` => MERGE PR #142 `--rebase --delete-branch` (docs-only, bot-merge allowed), verify main advance past 630dce19bf, verify-and-dispatch pages if silent, close nothing (body uses non-closing keywords). `fix` findings => route to `lab` (infra guard: never fix/continue on infra PR). Tester death => error-class inspection (one same-error retry max, then lab with run IDs).
