...
- **Updated:** 2026-08-23 (maintainer run 32624977425, owner `/oc continue`+`/oc maintainer` on PR #121). Re-survey confirms: **B9/B10 + polish shipped on PR #121** (build head `8ca7b52`) - WebP/TIFF/ICC front-end wrappers + real-Kodak bench harness (durable CSV, SHA pin, numeric gate), 23/23 gtest + fuzz 1000 PASS, warning-clean. The owner's `/oc continue` at 07:12:51Z launched **B11 build `32624970947` (in flight, in_progress)** on PR #121 - the real Kodak measurement. A **duplicate opencode run `32624977415` is also PENDING** on `opencode-121` (redundant second `/oc continue`; cancel-in-progress:false means it queues, not cancels). PR #121 MERGEABLE, kept OPEN (no premature #117 close). PR #118 is FULLY IDLE (B5.48, 0%).
...
- **CONCURRENCY NOTE:** `opencode-${{issue}}` and `maintainer-${{issue}}` are SEPARATE concurrency groups (both `cancel-in-progress: false`). They do NOT cancel each other. A genuine duplicate is only created by posting a SECOND `/oc continue`/`/oc build` on the SAME issue while one is already in flight - which happened this run (`32624970947` + pending `32624977415` on #121).
- **KODAK CORPUS UNBLOCK (verified present this run):** the real Kodak 24-image benchmark set ALREADY exists in-repo at `obsidian/benchmarks/data/kodak/kodim01.ppm … kodim24.ppm` (24 images, the Obsidian benchmark corpus). Prism's `prism/benchmarks/data/` only ships `kodak.sha256` (no images), which is why builds reported the gate "blocked on external dataset" - that claim is NOW FALSE. The gate IS measurable: point `prism bench --effort 3 --kodak <checkout>/obsidian/benchmarks/data/kodak` (or copy the 24 images into `prism/benchmarks/data/kodak/`) to produce a REAL Kodak durable CSV and measure M3 < 8.71 bit-exactly. The progress file still says "blocked" - the B11 build must override it.

## IN FLIGHT (builds)
- **PR #121 (Architect blueprint + live code, branch `opencode/issue117-20260823061608`):** head `8ca7b52` (B9/B10+polish shipped: WebP/TIFF/ICC front-end + real-Kodak bench harness durable CSV; 23/23 gtest + fuzz 1000 PASS, warning-clean, synthetic-only CSV so far). **B11 build `32624970947` IS IN FLIGHT** (in_progress, started 07:12:54Z), driven by owner `/oc continue` at 07:12:51Z. B11 = run REAL Kodak on the in-repo corpus (`obsidian/benchmarks/data/kodak`) to finally measure the M3 < 8.71 gate bit-exactly. This is the live, owner-driven build - NOT duplicated by this maintainer run (decision `[]`).
  - A **duplicate run `32624977415` is PENDING** on `opencode-121` (07:13:03Z) - a redundant second `/oc continue` on the same issue. With cancel-in-progress:false it queues behind `32624970947` and will likely re-run the same real-Kodak measurement; harmless but wasteful. No third trigger posted by me.
...
- **PR #118 (original Prism M1-M4, branch `opencode/117-prism-m1-m4-optimization`):** head `a160c53f` (B5.48, 11.025 bpp, 0% - saturated dead end). Last build `32624540354` COMPLETED (07:03:15Z). **FULLY IDLE now** - no in-flight work. Dropped from rotation (never delete branch); do not re-continue #118 (0% gains). Fold any residual wins into #121 later.

## PENDING (in order)
    1. **Let B11 build `32624970947` run** (owner-driven, not duplicated). B11 MUST point the bench harness at the REAL Kodak corpus (`obsidian/benchmarks/data/kodak`, 24 images) to produce a durable `prism/benchmarks/results/*.csv` and measure M3 < 8.71 bit-exactly. Ignore the stale "blocked on external dataset" note in the progress file.
...
## NEXT STEPS
    1. **B11 build on #121** (`32624970947`, owner-driven) runs to completion - REAL Kodak measurement (point harness at `obsidian/benchmarks/data/kodak`). Not duplicated by this maintainer run (decision `[]`).
...
## OPEN QUESTIONS
- After B11 lands, is there a REAL Kodak `prism/benchmarks/results/*.csv` with mean < 8.71 bit-exactly? The in-repo `obsidian/benchmarks/data/kodak` corpus (24 images) removes the prior "external dataset" blocker; the progress file still falsely says "blocked" - the build must override it. If B11 still only posts synthetic/probe numbers, escalate to `research` only if the real corpus genuinely fails to encode/decode.
- Will the in-flight B11 build `32624970947` run to completion (the duplicate `32624977415` queues behind it on `opencode-121`, harmless)?
