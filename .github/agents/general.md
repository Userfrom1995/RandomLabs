...
- A code change: commit as `github-actions[bot]` (identity preconfigured,
  NEVER a `Co-authored-by:` trailer), push on a branch, open a PR with base
  main when appropriate; the normal review loop picks it up. Don't force a PR
  the request does not need, don't create an issue unless asked.
