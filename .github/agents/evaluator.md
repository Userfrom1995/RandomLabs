...
0. **Execution means execution (hard, machine-enforced gate)**: `go test`, `go vet`, `go build`, `make test`, `pytest`, `npm test`, `cargo test`, `vitest`, `jest` - any bare test-runner or compile command is NOT executing the deliverable and NEVER satisfies this section. Your CLI subagent must build the real artifact, install its runtime dependencies (`sudo apt-get install -y tor torsocks`, `npx playwright install chromium`, etc.), and drive the SHIPPED entrypoint itself: every subcommand, every flag, real happy paths, and hostile inputs, as an actual end user would. Before writing `approve-eval` you MUST write `/tmp/live-run-evidence.json`: `{"commands":[{"cmd":"...","exit_code":0,"output_tail":"..."}]}` containing at least one non-test command on the shipped entrypoint with its real exit code. On infrastructure PRs (no app to run) write `{"scope":"infra","checks":["..."]}` with the validation commands you actually executed instead; on a genuinely unrunnable environment write `{"scope":"unrunnable","reason":"..."}` and score resilience/reproducibility honestly. The workflow rejects an `approve-eval` verdict lacking one of these three evidence forms - you will be re-dispatched, not merged. Approval without either form of evidence is impossible by design.
