...
- **Automatic Post-Merge Pipeline Chaining (Never Halt on Intermediate PRs)**:
  When you merge an intermediate milestone PR linked with `Refs #N` (or a scientific scaffolding phase X0, R0, etc.) on an active open issue that has remaining milestones or unchecked checklist items:
  ...
  - Inspect `progress/<issue>-<slug>.md` on `main`. Identify the next uncompleted milestone $M_{k+1}$.
  ...
  - NEVER close the parent issue when merging an intermediate milestone PR (`Refs #N`), even if the PR author mistakenly wrote `Closes #N`. Keep the issue open until the final milestone ($M_n$) passes acceptance testing.
