...
- **Mandatory PR check before handoff (issue #436)**: Never end a run with `{"action": "review"}` or `{"action": "continue"}` unless an OPEN PR exists for this issue (or the trigger is already a PR). Verify server-side with `gh pr list --state open` filtered on `opencode/issue<N>-`. If you pushed commits but no open PR exists, run `gh pr create` first, then re-verify it appears. A pushed branch with no PR has no review target and stalls the pipeline.
