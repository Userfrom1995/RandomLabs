...
6. **Linked issue** - the PR body references a dedicated task issue with `Refs #N` (for intermediate phases) or `Closes #N` (only when all epic requirements are completely satisfied; never close on partial or failing work; never a universal/meta issue).
7. **Ideas entry** - an `ideas/YYYY-MM-DD-<name>-<what-is-it>.md` writeup with
   a unique, non-generic name.
8. **Docs & site** - The root `/docs/` folder is strictly for global lab documentation and must NEVER be overwritten. Project-specific documentation must be placed in `/<project-name>/docs/`. EVERY project must ship `/<project-name>/index.html` served on GitHub Pages: the live entrypoint for interactive/static projects, or an introduction and documentation hub (real quickstart, command reference, honest support matrix, links to `/<project-name>/docs/`) for CLI, engine, backend, and library projects that cannot run on Pages. A project directory WITHOUT an `index.html` is a blocking finding (Owner directive). The root `index.html` landing must be updated with links to the new project and its docs, and the project's Website links (landing card + README entry) must resolve without 404s.
9. **Preview infra** - the PR-preview feature in `pages.yml` intact.
10. **No-interactive-input** - no blocking CLI prompts (`input()`, `raw_input`, `prompt()`, `readline`, `select`) in headless/shipped batch code. For web applications, interactive visual direct manipulation in the browser is required.
11. **Progress-file honesty** - if the PR has `progress/*.md`, does the
     checklist match reality? (Status complete with unchecked boxes -> finding. For Phase Epic PRs referencing `Refs #N`, only the active phase checklist items must be checked `[x]`; unchecked boxes for future phases are expected and permitted. Active phases MUST carry semantic names (`Phase <k>: <Capability>`); bare robotic numbers (`M1`, `M2`, bare `Milestone 1`) are a finding.)
...
16. **Phase Slicing Discipline** - Reject monolithic PRs attempting to implement dozens of disparate capabilities at once (>7 features) instead of following the Architect's vertical phase roadmap. Reject phase PR titles or branch suffixes using bare robotic numbers (`M1`, `M2`, bare `Milestone 1`); demand semantic `Phase <k>: <Capability>` names. Output `{"action": "fix"}` instructing the Builder to prune down to the Active Phase slice.
...
18. **Zero Milestone Leakage in Documentation (Static Check, Blocking)** - Scan every new or modified file under `<project>/docs/` and `<project>/README.md` for internal development markers. Flag as a blocking finding any occurrence (case-sensitive where noted, otherwise case-insensitive) of: bare milestone codes (`M1`, `M2`, `M3`, `M4 pointer`), bare counters (`Milestone 1`, `this milestone`), sprint references, or per-phase changelog headers. Exception: performance-gate labels that are part of a binding empirical ledger quoted verbatim from `progress/` or `decisions/builder/` (e.g. a measured gate table) are permitted ONLY inside project-internal research notes, never as documentation structure. Demand the fix: rewrite the affected doc sections as one unified product view with no phase-stamped headers.
