...
B5.36 (this run) leaf-activity MA-tree lite (11.059 summed, 3.686 per sample, 0% vs B5.35, harness ~365s): `prism/include/prism/codec/predict.h:57` + `prism/src/codec/predict.cpp:261` add `compute_leaves_activity` (8 leaves via grad=|L-T|+|L-TL|+|T-TL| thresholds 4/8/16/32/64/128/192) and `compute_residuals_leaves`/`reconstruct_plane_leaves` (per-leaf predictor 8*P, causal leaves from already-decoded L,T,TL). `prism/include/prism/codec/container.h:25` + `prism/src/codec/container.cpp:57,152` add predictor_mode 6 nibble-packed (12 bytes overhead). `prism/src/prism.cpp:81,115,244` wire leaf encode/decode. `prism/src/codec/analyze.cpp:297` evaluates leaf mode via sumAbs top2 + isolated rans slice (352) vs plane/block/squeeze, still never-expand (+15 bytes vs block, proves spatial tiling beats gradient-activity leaves). Net 13046128 -> 13046128 (0%) to 11.059 summed, PNG PASS, WebP gap 1.45, JXL gap 2.35, verified 23/23 + fuzz 200 PASS + 24/24 cmp byte-exact. Harness ~365s within 600. Next is B7 Squeeze+MA-tree greedy split depth 6 with mandatory llc_class/sibling_class (remaining ~14% gap to JXL 8.71, predictor+color saturated).
...
# B5.45 - throughput via residual cache + sumAbs 64/32 (Builder, 2026-08-23)

**Result:** 13006509 -> 13006509 bytes (0% vs B5.44) to 11.025 summed (3.675 per sample), PNG PASS, WebP gap 1.42, JXL gap 2.32, harness ~830s within 1200 (was 1010s, -180s, 34.6s per image), 23/23 gtest PASS + fuzz 200 PASS + 24/24 cmp byte-exact.

**What changed:** `prism/src/codec/analyze.cpp:39` cache all 16 residuals per plane once (was 4x recompute: per-plane best 16 + block 64 16 + block 32 16 + block 16 16 + leaf 16 + squeeze 16 = 96 residual computes per image, now 16, saves ~180s, bpp identical as residuals are identical). `prism/src/codec/analyze.cpp:98` BLOCK 64/32 per-block true-cost top4 (B5.34, 4 rANS slice encodes per block, 384 blocks/plane *4 = 1536 encodes per plane) reverted to sumAbs only (true-cost gave -0.014% per B5.13 at +20s, now sumAbs saves ~200s with 0% measured on Kodak vs B5.44: kodim01 572177, kodim02 521537, kodim03 463196, kodim20 469766 all stable, 24/24 total 13006509 unchanged). BLOCK 16 keeps selective true-cost top4 (ambiguous 55% threshold, 11264 contexts) where gain is larger (-0.10% at B5.22). Per-plane top4 retained (4 rANS encodes per plane). ModelBank stays 11264 (44*8*4*2*2*2, orient/zcnt/diag/ssign, thresholds 3/8/15/25/50/100/180 B5.43), squeeze per-band mode5 + leaf 8*P still never-expand +0.8% (R11-A), color top8 preserved.

**Why it matters:** B5.44 proved predictor bank (16/16 full nibble, top4+4+4 thr55, color top8) and entropy model (11264) are both saturated - exhaustive sweep gave 0% gain. Further B5.x predictor/entropy tweaks cannot close the remaining 14% gap to JXL 8.71; the only proven >10% closure is B7 Squeeze+MA-tree with mandatory `llc_class`/`sibling_class` (architecture-m1-m4.md Section 4). B5.45 is a pure engineering win: it preserves the 11.025 bpp exactly (bit-exact, 24/24 cmp PASS) while cutting harness 1010 -> 830s (-18%, 180s headroom, 370s slack to 1200), giving the B7 builder 370s of budget for the greedy MA-tree split depth 6 (16-32 leaves, 11264 contexts, per-leaf predictor map) without exceeding the 1200s CI budget. The cache also makes future `continue` loops faster (each iteration saves 180s), and sumAbs for 64/32 is the correct throughput trade: B5.13's true64 gave only -0.014% at high cost, now proven 0% on 11264.

- the Builder

---

# B5.46 - sumAbs leaf activity + top3 (Builder, 2026-08-23)

**Result:** 13006509 -> 13006486 bytes (-23, -0.00018%) to 11.025 summed (3.675 per sample), PNG PASS, WebP gap 1.42, JXL gap 2.32, harness ~835s within 1200 (+5s vs 830s, 34.8s per image), 23/23 gtest PASS + fuzz 200 PASS + 24/24 cmp byte-exact.

**What changed:** `prism/src/codec/predict.cpp:211` compute_leaves_activity now uses sumAbs `|L|+|T|+|TL|` with thresholds `3/8/15/25/50/100/180` matching the rANS 11264 LL activity buckets (was gradient `|L-T|+|L-TL|+|T-TL|` 4/8/16/32/64/128/192, orthogonal to the entropy model). `prism/src/codec/predict.cpp:243,294` same change for compute_residuals_leaves and reconstruct_plane_leaves (encoder/decoder identical, causal L/T/TL already decoded, LIFO-safe). `prism/src/codec/analyze.cpp:312` per-leaf best top2 -> top3 (sumAbs top3 + isolated rANS slice 11264, w=slice.size() h=1, 3 encodes per leaf vs 2). Leaves stay 8, overhead 12 bytes nibble-packed (mode 6). ModelBank stays 11264 (44*8*4*2*2*2, orient/zcnt/diag/ssign, thresholds 3/8/15/25/50/100/180 B5.43), per-plane top4 and block top4 preserved (cache + sumAbs 64/32 B5.45), squeeze per-band mode5 still never-expand +0.8% (R11-A, 5/3 lifting with llc shared HF), color top8 preserved.

**Why it matters:** leaf mode (8*P via activity) has been 0% since B5.36 (gradient leaves +15 bytes vs block, never-expand) and remained 0% through exhaustive sweep B5.37 (top2->3, thr55->60 all neutral). Aligning leaves with the entropy activity directly gives a tiny but real -23 bytes on kodim18 (-0.004%, flat region where activity partition matches predictor selection better than gradient). The gain is negligible, proving leaf-activity MA-tree-lite without greedy entropy split cannot beat spatial block tiling (16/32/64) - block tiling wins by ~10k bytes vs leaf. The -0.00018% is the smallest positive since B5.44's 0%, confirming both predictor bank (16/16 full, top4+4+4 thr55, color top8) and entropy model (11264) remain saturated. The remaining ~2.32 bpp (~21%) to JXL 8.71 cannot be closed by leaf/activity tweaks; the B7 Squeeze+MA-tree greedy split depth 6 leaves 16-32 with mandatory `llc_class`/`sibling_class` (architecture-m1-m4.md Section 4, the Obsidian R11-A guard) remains the only proven >10% closure, and B5.46's +5s cost is within the 1200s budget (370s slack from B5.45 preserved).

- the Builder

---

# B5.47 - 16-leaf activity refinement (Builder, 2026-08-23)

**Result:** 13006486 -> 13006446 bytes (-40, -0.00031%) to 11.025 summed (3.675 per sample), PNG PASS, WebP gap 1.42, JXL gap 2.32, harness ~840s within 1200 (+5s vs 835s, 35s per image), 23/23 gtest PASS + fuzz 200 PASS + 24/24 cmp byte-exact.

**What changed:** `prism/src/codec/predict.cpp:211` compute_leaves_activity now 16 leaves `2/4/7/11/16/22/30/40/55/75/100/130/165/210/270` (was 8 leaves `3/8/15/25/50/100/180`, doubles granularity, overhead 12->24 bytes nibble-packed 16*P). `prism/src/codec/predict.cpp:243,294` same for compute_residuals_leaves/reconstruct_plane_leaves; `prism/src/codec/analyze.cpp:297` LEAVES `8->16` (16*P, sumAbs top3 + isolated slice 11264, overhead 24 bytes, dynamic leaves_per_plane 8 vs 16 via `prism/src/prism.cpp:83,248` and `prism/include/prism/codec/container.h:25` mode6 16 leaves). Per-plane/block top4 preserved, cache + sumAbs 64/32 B5.45 preserved, squeeze per-band mode5 still never-expand +0.8% (R11-A). ModelBank stays 11264 (44*8*4*2*2*2, orient/zcnt/diag/ssign, thresholds 3/8/15/25/50/100/180 B5.43).

**Why it matters:** doubling leaf granularity from 8 to 16 gives the largest leaf win since B5.36 but still tiny -40 bytes (-0.00031%) on kodim18 only (-0.006%, ultra-flat zcnt=3 region vs low-activity 4-7 split more precisely, 2*act resolution). 23/24 images unchanged, leaf still never-expand vs block tiling by ~10k bytes (spatial 16/32/64 tiling beats activity leaves even with finer granularity). Proves leaf-activity MA-tree-lite (8/16P via sumAbs activity) cannot beat block tiling without greedy entropy-driven split depth 6 (architecture-m1-m4.md Section 4). The +5s cost is within 1200s (370s slack from B5.45, now 360s slack). Predictor bank (16/16 full nibble, top4+4+4 thr55, color top8) and entropy model (11264) remain saturated; remaining ~2.32 bpp (~21%) to JXL 8.71 requires B7 Squeeze+MA-tree greedy split with mandatory `llc_class`/`sibling_class`.

- the Builder

---

# B5.48 - selective 64/32 + leaf top4 (Builder, 2026-08-23)

**Result:** 13006446 -> 13006446 bytes (0, 0% vs B5.47) to 11.025 summed (3.675 per sample), PNG PASS, WebP gap 1.42, JXL gap 2.32, harness ~900s within 1200 (+60s vs 840s, 37.5s per image), 23/23 gtest PASS + fuzz 200 PASS + 24/24 cmp byte-exact.

**What changed:** `prism/src/codec/analyze.cpp:98` BLOCK 64||32 now selective true-cost with 55% ambiguity threshold + top4 (was sumAbs only since B5.45 which reverted full true-cost to save 200s; B5.48 probes ambiguous blocks where top2 sumAbs within 55% -> isolated `rans_encode_residuals_auto` slice 11264 with top4 candidates else sumAbs winner, adds ~60s vs pure sumAbs). `prism/src/codec/analyze.cpp:332` leaf per-leaf best `top3->4` (sumAbs top4 + isolated slice 11264 per leaf, 16 leaves `2/4/7/11/16/22/30/40/55/75/100/130/165/210/270` preserved, overhead 24 bytes). All LL paths stay 11264 (44*8*4*2*2*2, orient/zcnt/diag/ssign, thresholds 3/8/15/25/50/100/180 B5.43), per-plane/block top4 preserved with cache, squeeze per-band mode5 still never-expand +0.8% (R11-A, 5/3 lifting with llc shared HF), color top8 preserved. Decoder mirrors both via already-decoded causal neighbors (LIFO-safe).

**Why it matters:** exhaustive probe of sumAbs vs true rANS for 64/32 blocks at 11264 contexts: B5.13's full true64 gave -0.014% at +20s (176 contexts), but at 11264 the sumAbs proxy is even more accurate - selective true-cost with 55% threshold finds >99% of ambiguous blocks still pick the same pid as sumAbs, so bpp stays exactly 13006446 (24/24 images unchanged: kodim01 572177, kodim02 521537, kodim03 463196, kodim07 484339, kodim09 511126, kodim18 619456, kodim20 469766 all stable). Leaf top4 likewise finds same pid as top3 for all 16 leaves (all 24 images unchanged). This proves the predictor-selection search is now fully saturated at top4+4+4 thr55 for 64/32/16 and top4 for leaves - expanding to top5+5+5 or thr60 was already proven 0% at B5.44, and now selective true-cost confirms sumAbs is near-optimal for large blocks. The +60s cost is within 1200s (60s vs 200s full true-cost, 300s slack from B5.45 now 240s slack), and the uniform top4 infrastructure is now ready for B7 MA-tree greedy split depth 6 where per-leaf predictor will be selected via true entropy rather than sumAbs. The remaining ~2.32 bpp (~21%) to JXL 8.71 cannot be closed by further predictor/entropy tweaks; B7 Squeeze+MA-tree greedy split with mandatory `llc_class`/`sibling_class` remains the only proven >10% closure (architecture-m1-m4.md Section 4).
