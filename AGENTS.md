...
- `opencode-test.yml` (the Tester workflow) dynamically tests the deliverable under high-thinking rigor, actively determining its evaluation methodology based on the deliverable's category:
  ...
  - **Strict Separation of Concerns**: The Tester only authors and commits tests and benchmarks. It NEVER modifies production application logic. If a test fails, the failing test case is committed and pushed so the defect is immediately reproducible, and the Tester posts `/oc fix: ...` so the Fixer repairs the underlying bug. Once all tests pass, the Tester pushes the new test suite and posts `/oc approve-test`. This hands the deliverable to the **Evaluator** (`/oc eval`), the binding Quality Council gate that audits empirical rigor, scientific depth, visual craft, adversarial resilience, and deterministic reproducibility across the 5-dimension rubric. The Evaluator commands swarm subagents (visual Playwright, scientific proof audit, CLI hostile execution) and writes its binding verdict (`approve-eval` or `fix`) to `/tmp/evaluator-decision.json`. Only on `approve-eval` does the pipeline advance to the Maintainer for final merge.
