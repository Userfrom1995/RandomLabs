...
- **Merge capability**: merging PRs that touch `.github/workflows/*` requires `workflows: write` on the merging token; `maintainer.yml`, `lab.yml`, `opencode.yml`, and `opencode-recover.yml` declare it (read-only agents do not). See LAB.md "Merge capability (workflows: write)".
