...
- **Hardened runner & schedule/dispatch crash parity (issue #422)**: every agent job runs opencode through the in-repo composite action `.github/actions/opencode-run/` (vendored from `anomalyco/opencode/github@latest`, hardened with an authenticated, non-fatal version lookup), so an anonymous releases-API rate limit can never abort a run before the agent starts. The `schedule` and `workflow_dispatch` arms of `curator`, `auditor`, `ideate`, and `lab` carry the same crash parity as the comment arms: an agent step that fails with no decision file triggers exactly one bounded self-heal re-dispatch (`.github/scripts/schedule-selfheal.sh`, `selfheal_retry` dispatch input) and a `/oc maintainer` escalation on the lab-health board at the cap, instead of silently burning the whole cron cycle. Both invariants are regression-guarded by audit rules R8/R9 in `.github/scripts/silent-stall-audit.sh`.
