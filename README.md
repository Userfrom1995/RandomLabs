...
- **Doom** (shipped, issue #362 closed) - a client-side web Doom engine at `/doom/`: checked WAD reader with layered diagnostics, WebGL paletted renderer with tier ladder, full desktop plus touch input with remapping, WebAudio SFX plus FM music, OPFS save persistence, multi-WAD load order with per-map isolation plus onboarding plus offline support (M1-M5 done, 424/424 node:test green, H1-H5 ledger measured end to end, merged via PR #367). See [ideas/2026-09-17-doom-client-side-web-engine.md](ideas/2026-09-17-doom-client-side-web-engine.md) * [Doom README](doom/README.md) * [Progress](progress/362-doom.md)

- **Umbra** (shipped, issue #375) - a Shadow Fight-inspired silhouette combat game at `/umbra/`: WebGPU WGSL shaders (silhouette, rim-light, particles, parallax backgrounds) with WebGL2 and Canvas2D fallback tiers, deterministic 60 Hz combat core (12-state machine, parry, chip, guard-break, hit-stop, combo scaling, seeded AI), 3 playable fighters plus 5 enemies across a 17-node story with dialogue, 3 phased bosses (summoner, duelist, eclipse), 6 weapons with frame-data tables and trail ribbons, dojo trials, shop economy with export/import, tutorial gate with graduation stipend, synth SFX plus adaptive arena music, hit sparks with KO slow-mo, and universal keyboard/gamepad/touch controls (455/455 node:test green, G1-G7 ledger measured end to end). See [ideas/2026-09-22-umbra-shadow-fight-webgpu-combat.md](ideas/2026-09-22-umbra-shadow-fight-webgpu-combat.md) * [Umbra README](umbra/README.md) * [Progress](progress/375-umbra.md)

- **torshim** (final review, issue #387 open) - a lightweight cross-platform Tor CLI in **Go** (stdlib only) at `/tor-cli/`: `torshim <app>` routes one app through Tor (torsocks shim on Linux, socks5h proxy env on macOS/Windows), `torshim shell` spawns a Tor-routed child shell, `sudo torshim connect` sends the whole Linux system through Tor (iptables/nft, snapshot-first, verify-gated, byte-exact restore), plus `disconnect`, `repair`, `status` (never claims protected when not), and `version`. M5 hardening adds a session lock, a torrc managed-key guard, and control-parser fuzzing; tri-OS CI is staged for Lab Engineer install. See [ideas/2026-09-23-torshim-m5-hardening-final.md](ideas/2026-09-23-torshim-m5-hardening-final.md) * [torshim README](tor-cli/README.md) * [Progress](progress/387-tor-cli.md)
