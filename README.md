...
- **Poolduel** (in progress, issue #302 open) - an exhaustive PostgreSQL connection pooler shootout at `/poolduel/`: pgagroal vs PgBouncer vs pgpool-II vs Odyssey vs pgcat plus a direct-PG control (Supavisor deferred with a written reason). One pooler-blind Python harness drives pgbench TPC-B-like, SELECT-only, churn, and prepared-statement workloads under `clients >> pool_size` with interleaved round-robin repeats and medians; M1 covers transaction pooling (7 cells, 9 CI chunks), M2 covers session/statement/I-O arms plus workload twins (52 measured rows plus 7 N/A rows, 16 CI chunks), M3 publishes the Pages report with best-vs-best plus iso-region comparisons, surface flatness, full-matrix CSVs, and a fairness audit. M1 medians (42 rows) and M2 medians (111 rows) plus the report bundle are published on main; M10 soak sweep is GREEN (34 medians: 27 measured plus 7 timeout/inconclusive over 3 cells x 6 arms x 2 tiers with 2 groups pending Maintainer re-dispatch, 156 raw); M11a statistics plus the M11b/M11c/M11d website rebuild merged; M12 reproducibility manifest merged (PR #343: harness/manifest.py, results/manifest.json, docs/digests.md, honestly empty docs/errata.md; full suite green); M13a soak-figure work is on main (soak visuals with absent-vs-timeout markers, tier-suffixed dossiers, loader guards; full suite green) and the M13 closure blueprint is filed, with the M13b final gate pending. Issue #302 stays open (Refs #302). See [ideas/2026-09-11-poolduel-shootout-report.md](ideas/2026-09-11-poolduel-shootout-report.md) * [Poolduel README](poolduel/README.md) * [Progress](progress/302-poolduel.md)
