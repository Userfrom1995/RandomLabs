...
- **Doom** (in progress, issue #362 open) - a client-side web Doom engine at `/doom/`: checked WAD reader with layered diagnostics, map discovery plus lump decoders plus texture composition, Hexen detect-and-report, stable engine boundary, 35 Hz fixed-step loop, Canvas2D automap first frame (M1 done, 35/35 node:test green). Full 3D rasterizer (M2), audio plus persistence (M3), and ecosystem polish (M4) follow. See [ideas/2026-09-17-doom-client-side-web-engine.md](ideas/2026-09-17-doom-client-side-web-engine.md) * [Doom README](doom/README.md) * [Progress](progress/362-doom.md)
