...
export function updateSleep(bodies, manifolds, joints, dt, deterministicNoSleep) {
    if (deterministicNoSleep)
        return;
    // build islands via union-find
    const n = bodies.length;
    const idToIdx = new Map();
    for (let i = 0; i < n; i++)
        idToIdx.set(bodies[i].id, i);
    ...
    for (const indices of islands.values()) {
        ...
        for (const idx of indices) {
            ...
            if (motion >= SLEEP_LINEAR * SLEEP_LINEAR) {
                allSlow = false;
                break;
            }
        }
