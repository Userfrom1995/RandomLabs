...
let gravityOn = true;
let lastManifolds: any[] = [];
let rng = mulberry32(seed);

function initWorld(scene: string): void {
  resetBodyIdCounter();
...
function spawn(type: string): void {
  const x = (rng() - 0.5) * 6;
  const y = 10 + rng() * 4;
  if (type === 'box') {
...
function wireUI(canvas: HTMLCanvasElement): void {
  ...
  document.getElementById('btn-reset')!.addEventListener('click', () => {
    const sel = (document.getElementById('scene') as HTMLSelectElement).value;
    initWorld(sel);
  });
  document.getElementById('btn-pause')!.addEventListener('click', () => { paused = !paused; (document.getElementById('btn-pause') as HTMLButtonElement).textContent = paused ? 'Resume' : 'Pause'; });
  ...
  document.getElementById('btn-gravity')!.addEventListener('click', () => {
    gravityOn = !gravityOn;
    world.gravity = gravityOn ? { x: 0, y: -9.81 } : { x: 0, y: 0 };
    (document.getElementById('btn-gravity') as HTMLButtonElement).textContent = `Gravity: ${gravityOn ? 'ON' : 'OFF'}`;
  });
  document.getElementById('chk-aabb')!.addEventListener('change', (e) => { showAABBs = (e.target as HTMLInputElement).checked; });
  document.getElementById('chk-contacts')!.addEventListener('change', (e) => { showContacts = (e.target as HTMLInputElement).checked; });
  document.getElementById('chk-vel')!.addEventListener('change', (e) => { showVelocities = (e.target as HTMLInputElement).checked; });
  document.getElementById('chk-sleep')!.addEventListener('change', (e) => { enableSleep = (e.target as HTMLInputElement).checked; world.deterministicNoSleep = !enableSleep; });
  document.getElementById('scene')!.addEventListener('change', (e) => {
    seed = Number((document.getElementById('seed') as HTMLInputElement).value) || seed;
    initWorld((e.target as HTMLSelectElement).value);
  });
  document.getElementById('seed')!.addEventListener('change', (e) => {
    seed = Number((e.target as HTMLInputElement).value) || 0;
    rng = mulberry32(seed);
    initWorld((document.getElementById('scene') as HTMLSelectElement).value);
  });
  ...
  canvas.addEventListener('click', (e) => {
    if (!(e.shiftKey || (document.getElementById('chk-joint') as HTMLInputElement)?.checked)) return;
    const rect = canvas.getBoundingClientRect();
    ...
    // find closest body
    let best: any = null; let bestD = 2;
    for (const b of world.bodies) {
      const d = Math.hypot(b.p.x - wx, b.p.y - wy);
      const rad = b.shape.kind === 'circle' ? b.shape.radius : 1.0;
      if (d < bestD && d < rad + 1) { bestD = d; best = b; }
    }
    if (!best) return;
    picked.push(best.id);
    if (picked.length === 2) {
      const mode = (document.getElementById('joint-type') as HTMLSelectElement).value;
      ...
      if (mode === 'revolute') {
        ...
        world.addJoint({ kind: 'revolute', A, B, localAnchorA: localA, localAnchorB: localB, accImpulses: [0, 0] } as any);
        ...
        world.addJoint({ kind: 'distance', A, B, localAnchorA: localA, localAnchorB: localB, length: len, accImpulses: [0] } as any);
      }
      picked = [];
    }
  });
  ...
  canvas.addEventListener('mousedown', (e) => {
    ...
    const scale = 32; const dpr = window.devicePixelRatio || 1;
    const W = canvas.width, H = canvas.height;
    const ox = W / (2 * dpr); const oy = H / dpr - 60;
    const wx = (sx - ox) / scale, wy = (oy - sy) / scale;
    for (const b of world.bodies) {
      if (b.isStatic) continue;
      const d = Math.hypot(b.p.x - wx, b.p.y - wy);
      const rad = b.shape.kind === 'circle' ? b.shape.radius : 0.9;
      if (d < rad + 0.5) { dragBody = b; dragStart = { x: wx, y: wy }; break; }
    }
  });
  canvas.addEventListener('mouseup', (e) => {
    if (!dragStart || !dragBody) { dragStart = null; dragBody = null; return; }
    const rect = canvas.getBoundingClientRect();
    const sx = e.clientX - rect.left, sy = e.clientY - rect.top;
    const scale = 32; const dpr = window.devicePixelRatio || 1;
    const W = canvas.width, H = canvas.height;
    ...
    if (dragBody.sleeping) { dragBody.sleeping = false; dragBody.sleepTimer = 0; }
    dragStart = null; dragBody = null;
  });
}
...
function main(): void {
  ...
  function loop(now: number): void {
    ...
    // telemetry
    (document.getElementById('fps') as HTMLElement).textContent = String(fps);
    (document.getElementById('body-count') as HTMLElement).textContent = String(world.bodies.length);
    (document.getElementById('checksum') as HTMLElement).textContent = hashState(world.bodies);
    (document.getElementById('ke') as HTMLElement).textContent = world.kineticEnergy().toFixed(3);
  }
  requestAnimationFrame(loop);
}
