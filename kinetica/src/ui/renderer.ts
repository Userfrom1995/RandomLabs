...
export function drawWorld(ctx: CanvasRenderingContext2D, world: World, opts: RenderOptions, manifolds?: any[]): void {
  ...
  for (const body of world.bodies) {
    if (body.shape.kind === 'circle') {
      ...
      ctx.beginPath(); ctx.moveTo(c.x, c.y); ctx.lineTo(ax, ay);
    ...
    if (opts.showAABBs) {
      ...
      if (body.shape.kind === 'circle') {
        minX = body.p.x - body.shape.radius; maxX = body.p.x + body.shape.radius;
        minY = body.p.y - body.shape.radius; maxY = body.p.y + body.shape.radius;
      } else {
...
export function worldFromScreen(canvas: HTMLCanvasElement, sx: number, sy: number): { x: number; y: number } {
